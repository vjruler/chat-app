    ...
    <div class="span8">
        <div class="well">
            <div class="chat-messages">
                <div class="btn btn-primary btn-history"><a href="<?php echo Yii::app()->createUrl('/history/' . $this->chat->uri)?>" target="_blank"><i class="icon-list icon-white"></i> See full story </a></div>
            </div>
        </div>
    </div>
    <div class="span3">
...
<div class="row">
    <div class="span8">
            <div class="form-inline well">
                <input type="text" class="input-xlarge" placeholder="message" id="message">
                <button class="btn" id="send-message"><i class="icon-comment"></i> to send </button>
                <br>
            </div>
    </div>
</div>

<div><em>To invite a friend to chat, just copy and send a link!</em></div>
...
<script type="text/javascript">
    ...
    socket.on('auth-result', function(data) {
        if (!data['result']){
            alert('Authentication error! Try entering again');
            ...
            socket.on('new-user', function(data){
                ...
                display_service_message('user ' + user['name'] + ' I went into chat')
            });
            ...
            socket.on('leave-user', function(data){
                ...
                if (!$("#user-" + user['id']).size())
                    return;
                $("#user-" + user['id']).remove();
                display_service_message('User ' + user['name'] + ' I went outto chat ')
            });
